...
opt_clf = tree.DecisionTreeClassifier(**best_params, random_state=random_seed)
...
train_scores, validation_scores = validation_curve(opt_clf, X_train, y_train, 'max_depth', param_range, cv=10, n_jobs=-1)
av_train_scores = np.mean(train_scores, axis=1)
av_validation_scores = np.mean(validation_scores,axis=1)
# VC Plot
